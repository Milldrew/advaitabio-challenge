...
export function colorlegend(target, scale, type, options) {
  ...
  legendBoxes
    ...
    .attr('x', function (d, i) {
      return (
        i * (boxWidth + boxSpacing) + (type !== 'ordinal' ? boxWidth / 2 : 0)
      );
    })
    .attr('y', function () {
      return boxHeight + 2;
    ...
    .text(function (d, i) {
      ...
      // show only the first and last for others
        ...
        if (i === 0) {
          return domain[0].toPrecision(2);
        }
        if (i === colors.length - 1) {
          return domain[domain.length - 1].toPrecision(2);
        }
      }
    });
...
export function wrapText(
  ...
  if (!x) {
    x = 0;
  } // optional x value to shift text
  if (!y) {
    y = 0;
  } // optional y value to shift text
  if (arr !== undefined) {
    ...
    for (i = 0; i < arr.length; i++) {
      ...
      if (arr[i + 1]) {
        linePxWNext = linePxWNext + 1 + arr[i + 1].length * charWidthPx;
      }
      myLine = myLine + ' ' + arr[i];
    ...
    if (outLines.length) {
      ...
      if (outLinesTot > maxLines) {
        outLinesTot = maxLines;
      }
